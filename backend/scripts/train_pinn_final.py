...
def train_final_model():
    ...
    print("   This specific model instance has no valid standalone accuracy number of")
    print("   its own -- its credibility comes from the held-out-split / cross-validated")
    print("   metrics already reported for this same recipe, not from any number computed")
    print("   here. Use pinn_phase2.pt (not this file) for any reported accuracy metric.\n")
    ...
    Y_dose_lin = df["target_dose_sv_per_particle_shrunk"].values * DOSE_SCALAR
    ...
    if n_floored > 0:
        print(f"ℹ️  {n_floored} rows have dose below the {DOSE_FLOOR_SV:.0e} Sv/particle floor "
              f"— clamped, not excluded.")
    ...
    weight_path = os.path.join(weights_dir, "pinn_phase2_final.pt")
    torch.save(model.state_dict(), weight_path)
    print(f"\n🎉 Final production model trained on 100% of data. Saved to: {weight_path}")
    print("   NOTE: pinn_phase2.pt (the held-out-validated model) is untouched --")
    print("   keep using that one for any reported accuracy metrics/plots.")
